...
RSpec.describe KFileset::FileSet do
  ...
  # then we are in recursive mode and the current path (parent directory) matches the previous entry
  # in the resulting array. This approach works as glob uses a depth-first search and not a breadth-first search,
  # as long as . is the first entry in the directory.
      ...
      context 'include deep folder / file pattern variations' do
        ...
        context 'when deep folder/all file and FNM_DOTMATCH (includes .DOT files, .DOT folders and folder/.DOTs)' do
          ...
          context 'with ignore folders - current' do
            let(:exclude) { KFileset::FileSet::IGNORE_LAMBDAS[:folder_current] }
          ...
          context 'with ignore folders - current_dot' do
            let(:exclude) { KFileset::FileSet::IGNORE_LAMBDAS[:folder_current_dot] }
  ...
  def match(type: nil, types: [], dot: nil, include_tags: [])
    ...
    filter_rules = filter_rules.select { |rule| types.include?(rule[:type]) } unless types.empty?
    ...
    filter_rules = filter_rules.select { |rule| include_tags.all? { |tag| rule[:tags].include?(tag) } } unless types.length.zero?
